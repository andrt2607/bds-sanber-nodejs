...
const addSiswa = async (req, res) => {
  try {
    ...
    if (adminIndex != -1 && users[adminIndex].isLogin == false) {
      ...
      let indexTrainer = users.findIndex(
        (trainer) => trainer.name == req.params.name
      );
      if (indexTrainer == -1) {
        ...
        res
          .status(200)
          .json({
            message: `Berhasil tambah siswa trainer ${req.params.name}`,
          });
    ...
    console.log("Error addSiswa : ", error);
  }
};
